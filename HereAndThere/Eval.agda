...
open import Data.Bool renaming (Bool to 𝔹) renaming (_∧_ to _∧𝔹_ ; _∨_ to _∨𝔹_)
open import Data.Sum.Base renaming (inj₁ to inl ; inj₂ to inr)
...
evalHT i (f ∨ g) = (evalHT i f) ∨𝔹 (evalHT i g)
evalHT i@(IHT h t p) (f ⇒ g) = ((evalHT i f) ⇒𝔹 (evalHT i g)) ∧𝔹
                               (evalC t (f ⇒ g))
...
total-ht-to-c : (t : IPC) → (f : F) → (b : 𝔹) → ((evalHT (THT t) f) ≡ b) →
                ((evalC t f) ≡ b)
...
total-c-to-ht : (t : IPC) → (f : F) → (b : 𝔹) → ((evalC t f) ≡ b) →
                ((evalHT (THT t) f) ≡ b)
...
-- (property 1)
here-to-there : (i : IPHT) → (f : F) → ((evalHT i f) ≡ true) →
                ((evalHT (THT (pt i)) f) ≡ true)
...
here-to-there i@(IHT h t p) (f ⇒ g) s =
  total-c-to-ht t (f ⇒ g) true (p2 (∧𝔹-to-× s))

-- rephrasing of property 1 for countermodels
-- <T,T> ⊭HT f implies <H,T> ⊭HT f
counter-there-to-here : (t : IPC) → (f : F) → ((evalHT (THT t) f) ≡ false) →
                        ((h : IPC) →
                         (p : (a : Var) → (h a ≡ true) → (t a ≡ true)) →
                         ((evalHT (IHT h t p) f) ≡ false))
counter-there-to-here t f c h p = contra {evalHT (IHT h t p) f}
                                         {evalHT (THT t) f}
                                         (here-to-there (IHT h t p) f) c
