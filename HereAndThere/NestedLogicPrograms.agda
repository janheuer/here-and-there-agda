...
nlp∧nlp-eq-nlp Π1 Π2 =
  nlp (nlpt Π1 ++ nlpt Π2) (NLP++NLPisNLP Π1 Π2) , Th∧Th-eq-Th++Th
...
factor⇒Th:[NR⇒NR] r (nlp (f ∷ _) (fisNR , _)) .((nrf r) ⇒ f) (inl refl) =
  (r , (nr f fisNR)) , refl
factor⇒Th:[NR⇒NR] r (nlp (_ ∷ Π) (_ , ΠisNLP)) ϕ (inr ϕ∈factor⇒Π) =
  let
...
replaceTh≡HT : (t : Th) → (τ : (f : F) → Σ F (λ g → f ≡HT g)) →
...
replace[f⇒f-eq-f∧f] : (f : F) → Σ F (λ g → f ≡HT g)
